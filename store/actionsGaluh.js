import { SET_USER, SET_BADGES, SET_ACTIVE_TARGET, TOGGLE_LOADER_PROFILE, SET_SPEND_BETWEEN } from './actionTypesGaluh.js'
...
export function getUserDetails(id) {
    ...
    return function(dispatch) {
        ...
        fetch(`http://3.83.144.143:3000/user/${userId}`)
...
export function getAllBadges() {
    return function(dispatch) {
        fetch(`http://3.83.144.143:3000/badge`)
        .then(response => response.json())
        .then(data => {
...
export function getUserActiveTarget(id) {
    let userId = +id
    return function(dispatch) {
        fetch(`http://3.83.144.143:3000/target/active/${userId}`)
        .then(response => response.json())
        .then(data => {
            if (data.length > 0) {
            dispatch(setActiveTarget(data[0]))
                ...
                fetch(`http://3.83.144.143:3000/transactions/between/${startDate}/${endDate}/${userId}/Expense`)
                .then(response => response.json())
...
export function addPushToken(pushToken, userId) {
    ...
    let id = +userId
    return function(dispatch) {
        fetch(`http://3.83.144.143:3000/user/pushtoken/${id}`, {
        method: "PATCH",
        headers: {
