...
export default function TargetProgress({activeTarget, spendingBetween}) {
    ...
    const progressPercentage = `${Math.round(totalSpend / activeTarget.monthlyTarget * 100)}%`
    ...
    return (
        <>
        <View style={{backgroundColor: '#DCDCDC', marginTop: 15, borderWidth: 3, borderColor: 'maroon', borderRadius: 20}}>
        ...
        <Text style={styles.targetTextBold}><Text style={{color:'black'}}>From {dateFormatter(start)} to {endDate} </Text>
            </Text>
        {
            new Date(activeTarget.startDate) > new Date() ?
            <View style={styles.container}>
                <Text style={{color:'darkred', fontSize: 16, fontWeight: 'bold'}}>No projection available yet</Text>
            </View>
            :
            <View style={styles.container}>
